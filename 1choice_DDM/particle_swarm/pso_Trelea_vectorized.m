...
[out,allRT] = feval(functname,pos);  % returns column of cost values (1 for each particle) %%% CHANGED TO ALSO OUTPUT SIMULATED RTs - PM
%---------------------------
...
% assign initial gbest here also (gbest and gbestval)
 ...
 gbestRT = allRT(:,idx1);   %%%%%%%%%%%%%%% ADDED IN TO KEEP TRACK OF BEST SIMULATED RT - PM 05/09/13
...
for i=1:me  % start epoch loop (iterations)
     ...
     [out,allRT] = feval(functname,[pos;gbest]);    %%%%%%% CHANGED TO ALSO OUTPUT SIMULATED RTs - PM 05/09/13
     outbestval  = out(end,:);
     out         = out(1:end-1,:);
    ...
    if rstflg == 0
     if minmax == 0
        ...
        if gbestval >= iterbestval
            ...
            gbestRT = allRT(:,idx1);   %%%%%%%%%%% ADDED IN TO UPDATE BEST SIMULATED RT DISTRIBUTION - PM 05/09/13
            % used with trainpso, for neural net training
            % assign gbest to net at each iteration, these interim assignments
            % are for plotting mostly
             if strcmp(functname,'pso_neteval')
...
% output & return
 ...
 varargout{3}=gbestRT;    %%%%%%%%%%%%%%% ADDED IN TO OUTPUT BEST SIMULATED RT DISTRIBUTION - PM 05/09/03
